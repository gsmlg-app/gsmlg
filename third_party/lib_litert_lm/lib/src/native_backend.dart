...
final class _NativeWorkerState {
  ...
  LiteRtLmResult<String> _generate(Map<Object?, Object?> args) {
    ...
    try {
      ...
      return LiteRtLmOk(_safePointerToDartString(textPtr.cast<ffi.Char>()));
    } catch (error) {
  ...
  void _generateStream(Map<Object?, Object?> args) {
    ...
    callback = ffi.NativeCallable<c.LiteRtLmStreamCallbackFunction>.listener((
      ...
      try {
        ...
        final errorText = errorMsg == ffi.nullptr
            ? null
            : _safePointerToDartString(errorMsg.cast<ffi.Char>());
        ...
        if (chunk != ffi.nullptr) {
          final bytes = _copyPointerToBytes(chunk);
          if (bytes.isNotEmpty) {
            active.bytesSink.add(bytes);
          }
        }
        if (isFinal) {
          active.events.send(<String, Object?>{
        ...
        if (active != null) {
          active.events.send(<String, Object?>{
            'type': 'error',
            'error': liteRtLmFailureToMap(
              LiteRtLmGenerationFailure('Callback error: $e'),
            ),
          });
        }
        scheduleMicrotask(() => _closeStream(streamId));
    ...
    streams[streamId] = _ActiveStream(
      sessionId,
      events,
      callback,
      onToken: (token) {
        if (token.isNotEmpty) {
          buffer.write(token);
          events.send(<String, Object?>{'type': 'token', 'text': token});
        }
      },
    );
...
final class _ActiveStream {
  _ActiveStream(
    ...
    bytesSink = utf8.decoder.startChunkedConversion(_TokenSink(onToken));
  }

  final int sessionId;
  final SendPort events;
  final ffi.NativeCallable<c.LiteRtLmStreamCallbackFunction> callback;
  late final ChunkedConversionSink<List<int>> bytesSink;
...
class _TokenSink implements Sink<String> {
  const _TokenSink(this.onToken);
...
Uint8List _copyPointerToBytes(ffi.Pointer<ffi.Char> ptr) {
  if (ptr == ffi.nullptr) {
    return Uint8List(0);
  }
  var length = 0;
  while ((ptr.cast<ffi.Uint8>() + length).value != 0) {
    length++;
  }
  if (length == 0) {
    return Uint8List(0);
  }
  final view = ptr.cast<ffi.Uint8>().asTypedList(length);
  return Uint8List.fromList(view);
}

String _safePointerToDartString(ffi.Pointer<ffi.Char> ptr) {
  if (ptr == ffi.nullptr) {
    return '';
  }
  final bytes = _copyPointerToBytes(ptr);
  if (bytes.isEmpty) {
    return '';
  }
