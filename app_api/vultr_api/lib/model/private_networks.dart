...
class PrivateNetworks {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is PrivateNetworks &&
    other.id == id &&
  ...
  @override
  String toString() => 'PrivateNetworks[id=$id, macAddress=$macAddress, ipAddress=$ipAddress]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PrivateNetworks[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PrivateNetworks[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PrivateNetworks> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<PrivateNetworks>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PrivateNetworks>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = PrivateNetworks.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
