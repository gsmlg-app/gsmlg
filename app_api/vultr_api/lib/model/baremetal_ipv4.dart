...
class BaremetalIpv4 {
  ...
  @override
  String toString() =>
      'BaremetalIpv4[ip=$ip, netmask=$netmask, gateway=$gateway, type=$type, reverse=$reverse, macAddress=$macAddress]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "BaremetalIpv4[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "BaremetalIpv4[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<BaremetalIpv4> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<BaremetalIpv4>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = BaremetalIpv4.listFromJson(
          entry.value,
          growable: growable,
        );
      }
