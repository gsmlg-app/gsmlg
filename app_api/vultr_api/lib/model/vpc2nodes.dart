...
class Vpc2nodes {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is Vpc2nodes &&
    other.id == id &&
  ...
  @override
  String toString() => 'Vpc2nodes[id=$id, ipAddress=$ipAddress, macAddress=$macAddress, description=$description, type=$type, nodeStatus=$nodeStatus]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Vpc2nodes[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Vpc2nodes[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Vpc2nodes> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<Vpc2nodes>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Vpc2nodes>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = Vpc2nodes.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
