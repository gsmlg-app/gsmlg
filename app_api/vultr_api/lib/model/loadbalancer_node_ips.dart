...
class LoadbalancerNodeIps {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "LoadbalancerNodeIps[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "LoadbalancerNodeIps[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<LoadbalancerNodeIps> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<LoadbalancerNodeIps>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = LoadbalancerNodeIps.listFromJson(
          entry.value,
          growable: growable,
        );
      }
