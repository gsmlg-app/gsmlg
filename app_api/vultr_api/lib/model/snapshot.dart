...
class Snapshot {
  ...
  @override
  String toString() =>
      'Snapshot[id=$id, dateCreated=$dateCreated, description=$description, size=$size, status=$status, osId=$osId, appId=$appId]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "Snapshot[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "Snapshot[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Snapshot> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<Snapshot>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Snapshot.listFromJson(
          entry.value,
          growable: growable,
        );
      }
