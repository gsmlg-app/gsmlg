...
class InstanceVpc {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is InstanceVpc &&
    other.id == id &&
  ...
  @override
  String toString() => 'InstanceVpc[id=$id, macAddress=$macAddress, ipAddress=$ipAddress]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "InstanceVpc[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "InstanceVpc[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<InstanceVpc> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<InstanceVpc>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<InstanceVpc>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = InstanceVpc.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
