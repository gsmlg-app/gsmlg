...
class HaltBaremetalsRequest {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is HaltBaremetalsRequest &&
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "HaltBaremetalsRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "HaltBaremetalsRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
      ...
      return HaltBaremetalsRequest(
        baremetalIds: json[r'baremetal_ids'] is Iterable
            ? (json[r'baremetal_ids'] as Iterable).cast<String>().toList(growable: false)
            : const [],
      );
    }
    return null;
  ...
  static List<HaltBaremetalsRequest> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<HaltBaremetalsRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<HaltBaremetalsRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = HaltBaremetalsRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
