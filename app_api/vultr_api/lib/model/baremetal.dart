...
class Baremetal {
  /// Returns a new [Baremetal] instance.
  Baremetal({
    this.id,
    this.os,
    this.ram,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is Baremetal &&
    other.id == id &&
    other.os == os &&
    other.ram == ram &&
    other.disk == disk &&
  ...
  @override
  String toString() => 'Baremetal[id=$id, os=$os, ram=$ram, disk=$disk, mainIp=$mainIp, cpuCount=$cpuCount, region=$region, defaultPassword=$defaultPassword, dateCreated=$dateCreated, status=$status, netmaskV4=$netmaskV4, gatewayV4=$gatewayV4, plan=$plan, label=$label, tag=$tag, osId=$osId, appId=$appId, imageId=$imageId, snapshotId=$snapshotId, v6Network=$v6Network, v6MainIp=$v6MainIp, v6NetworkSize=$v6NetworkSize, macAddress=$macAddress, tags=$tags, userScheme=$userScheme]';
  ...
  /// Returns a new [Baremetal] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static Baremetal? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Baremetal[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Baremetal[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Baremetal> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <Baremetal>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = Baremetal.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, Baremetal> mapFromJson(dynamic json) {
    final map = <String, Baremetal>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Baremetal.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of Baremetal-objects as value to a dart map
  static Map<String, List<Baremetal>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Baremetal>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = Baremetal.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
