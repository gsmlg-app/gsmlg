...
class Baremetal {
  ...
  @override
  String toString() =>
      'Baremetal[id=$id, os=$os, ram=$ram, disk=$disk, mainIp=$mainIp, cpuCount=$cpuCount, region=$region, defaultPassword=$defaultPassword, dateCreated=$dateCreated, status=$status, netmaskV4=$netmaskV4, gatewayV4=$gatewayV4, plan=$plan, label=$label, tag=$tag, osId=$osId, appId=$appId, imageId=$imageId, snapshotId=$snapshotId, v6Network=$v6Network, v6MainIp=$v6MainIp, v6NetworkSize=$v6NetworkSize, macAddress=$macAddress, tags=$tags, userScheme=$userScheme]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "Baremetal[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "Baremetal[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Baremetal> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<Baremetal>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Baremetal.listFromJson(
          entry.value,
          growable: growable,
        );
      }
