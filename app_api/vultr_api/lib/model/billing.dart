...
class Billing {
  ...
  @override
  String toString() =>
      'Billing[id=$id, date=$date, type=$type, description=$description, amount=$amount, balance=$balance]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "Billing[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "Billing[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Billing> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<Billing>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Billing.listFromJson(
          entry.value,
          growable: growable,
        );
      }
