...
class RegionApi {
  ...
  Future<Response> listAvailablePlansRegionWithHttpInfo(String regionId, { String? type, }) async {
  ...
  /// * [String] type:
  ///   Filter the results by type.  | **Type** | **Description** | |----------|-----------------| | all | All available types | | vc2 | Cloud Compute | | vdc | Dedicated Cloud | | vhf | High Frequency Compute | | vhp | High Performance | | voc | All Optimized Cloud types | | voc-g | General Purpose Optimized Cloud | | voc-c | CPU Optimized Cloud | | voc-m | Memory Optimized Cloud | | voc-s | Storage Optimized Cloud | | vbm | Bare Metal | | vcg | Cloud GPU | 
  Future<ListAvailablePlansRegion200Response?> listAvailablePlansRegion(String regionId, { String? type, }) async {
    final response = await listAvailablePlansRegionWithHttpInfo(regionId,  type: type, );
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'ListAvailablePlansRegion200Response',) as ListAvailablePlansRegion200Response;
  ...
  Future<Response> listRegionsWithHttpInfo({ int? perPage, String? cursor, }) async {
    // ignore: prefer_const_declarations
  ...
  /// * [String] cursor:
  ///   Cursor for paging. See [Meta and Pagination](#section/Introduction/Meta-and-Pagination).
  Future<ListRegions200Response?> listRegions({ int? perPage, String? cursor, }) async {
    final response = await listRegionsWithHttpInfo( perPage: perPage, cursor: cursor, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
