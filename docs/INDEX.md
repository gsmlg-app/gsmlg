# Documentation Index

## Quick Links

| Document | Description |
|----------|-------------|
| [BRICKS.md](BRICKS.md) | Mason bricks guide -- code generation templates for screens, BLoCs, widgets, plugins, and more |
| [BRICK_TESTING.md](BRICK_TESTING.md) | How brick-generated code is tested |
| [TESTING.md](TESTING.md) | Running tests across the monorepo |
| [LOCAL_MODELS.md](LOCAL_MODELS.md) | Downloading preset GGUF local models from Hugging Face |
| [DOCUMENTATION.md](DOCUMENTATION.md) | Error handling and logging system architecture |
| [USAGE.md](USAGE.md) | Error handling system setup and usage |
| [FORM_BLOC.md](FORM_BLOC.md) | Form management with DuskMoon Form and BLoC |
| [ISOLATE_ORCHESTRATOR.md](ISOLATE_ORCHESTRATOR.md) | Managed Dart isolate pools for long-running background workers |
| [DEPLOYMENT_SETUP.md](DEPLOYMENT_SETUP.md) | Play Store and App Store deployment secrets configuration |
| [MONITOR_PRD.md](MONITOR_PRD.md) | Monitor module product requirements |
| [MONITOR_DESIGN.md](MONITOR_DESIGN.md) | Monitor module technical design |
| [MONITOR_IMPLEMENTATION.md](MONITOR_IMPLEMENTATION.md) | Monitor module implementation guide |

## See Also

- [README.md](../README.md) -- Project overview and quick start
- [CLAUDE.md](../CLAUDE.md) -- AI assistant guidance for this codebase
